<script>
  ...
  import {
    goto
  } from "@roxi/routify/runtime/helpers";
  ...
  function populate() {
    ...
    var sample_image_question = document.getElementById("sample_image_question");
    ...
    if (find.length > 0) {
      ...
      selectedAnswer = find[0].answer.toLowerCase();
    }
    ...
    if (navigator.onLine) {
      if (selectedAnswer) {
        ...
        const callback = (response) => {
          ...
          timer = response.data.time_remaining
          questionId = null;
          selectedAnswer = null;
          questionIndex++;
          populate();
        ...
        ANSWER_QUESTION(questionId, data, callback, onError);
      } else {
        questionId = null;
        selectedAnswer = null;
        questionIndex++;
  ...
  function endExam() {
    ...
    END_EXAM(currentPaper.paper_id, callback, onError);
  }
...
<main>
  ...
  <button id="read" class={`bg-blue-400 block p-3 text-white cursor-pointer absolute top-200 right-10`}
    style="z-index: 100;" on:click={()=> handlePassage()}
  ...
  <div class="w-full container rounded mt-32 mx-32 p-3 center-box">
    ...
    <div class="absolute top-0 right-10 flex flex-col ">
      <div class="relative pt-1">
        <!-- <h1 class="font-16 text-black text-2xl font-bold my-2 pull-right">Timer: <span>{timer ?
            convertTime(Math.floor(timer * 60)) : ''}</span></h1> -->
        <br>
        <br>
        <br>
        ...
        <div class="overflow-hidden h-2 mb-4 text-xs flex rounded bg-blue-200" style="width: 300px">
          <div style={`width:${ Math.ceil(anwsered.length / questions?.length * 100) }%`}
    ...
    <div class="question" data-intro="Question Block">
      <div class="flex flex-col mx-5">
        ...
        <img id="sample_image" class="my-2" src width="500px" height="500px" />


        <p style="height: 200px; transition: all 0.5s ease-in-out"
          class="font-14 text-black overflow-y-scroll border border-black border-4 p-2 round-sm" id="passage"></p>
        ...
        <p class="font-14 text-black" id="question" data-title="Instructions"
          data-intro="Read through questions carefully"></p>
        <img id="sample_image_question" class="my-2" src width="200px" height="200px" />

        <br>
        <br>
        <div class="flex flex-wrap justify-between ">
          <button class={`button button--option ${selectedAnswer==='a' ? 'button--option--active' : '' }`} id="btn0"
            on:click={()=> {selectAnswer(getQuestionIndex().answer,'a'), next()} } data-title="Instructions"
            data-intro="Select desired Answer"> <span id="choice0"></span>
          </button>
          <button class={`button button--option ${selectedAnswer==='b' ? 'button--option--active' : '' }`} id="btn1"
            on:click={()=> {selectAnswer(getQuestionIndex().answer,'b'),next()} }> <span id="choice1"></span>
          </button>
          <button class={`button button--option ${selectedAnswer==='c' ? 'button--option--active' : '' }`} id="btn2"
            on:click={()=> {selectAnswer(getQuestionIndex().answer,'c'),next()} }> <span id="choice2"> </span>
          </button>
          <button class={`button button--option ${selectedAnswer==='d' ? 'button--option--active' : '' }`} id="btn3"
            on:click={()=> {selectAnswer(getQuestionIndex().answer,'d'),next()} }> <span id="choice3"></span>
          </button>
        </div>

        {#if parseInt(anwsered.length) === parseInt(questions?.length)}
          <div class="flex justify-end">
          <button class={`bg-blue-500 text-white py-3 px-5
            rounded-sm outline-none cursor-pointer hover:bg-blue-700`} on:click={() => endExam()}>Finish Exam</button>
        </div>
        {:else}
        <div class="flex justify-between">
          <button class={`${questionIndex < 1 ? 'bg-gray-400' : 'bg-blue-500' } text-white py-3 px-5 rounded-sm
            outline-none cursor-pointer`} on:click={()=> prev()}
            >
            Previous</button>
          <button class={`${questionCount - 1> questionIndex ? 'bg-blue-500' : 'bg-gray-400' } text-white py-3 px-5
            rounded-sm outline-none cursor-pointer hover:bg-blue-600`} on:click={() => next()}>Next</button>
        </div>
        {/if}
  ...
  <div class="w-full container rounded mx-32 p-3 center-box">
    <div class="flex flex-wrap">
      ...
      {#each pages as page,i }
        <button class={`button button--paginated ${findIndex(anwsered,page.id) === page.id ? 'button--paginated__answered ' : ''}  ${questionIndex === i ? 'button--paginated__active' : ''}`}  on:click={() => selected(i)}>{i+ 1}</button> 
     {/each}
  ...
  <div class="flex py-5 mx-auto">
    <div class="flex">
      <button class="bg-blue-500 py-1 px-2 "></button>
      <span class="text-black mx-2">Primary Color</span>
    </div>
    <div class="flex">
      <button class="bg-green-500 py-1 px-2"></button>
      <span class="text-black mx-2">Answered Question</span>
    </div>
    <div class="flex">
      <button class="bg-gray-300 py-1 px-2"></button>
